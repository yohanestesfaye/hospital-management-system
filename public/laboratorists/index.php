...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        if (!$errors) {
            ...
            $stmt->execute([$username, $name, $hash, 'Laboratorist', $license_number, $certification, $shift, $desk_number, $specialty_area]);
            redirect('/hms/public/laboratorists/index.php?msg=created');
        }
        ...
        if (!$errors) {
            ...
            redirect('/hms/public/laboratorists/index.php?msg=updated');
        }
    } elseif ($action === 'delete') {
        $id = (int)post('id');
        $stmt = $db->prepare('DELETE FROM users WHERE id = ? AND account_type = ?');
        $stmt->execute([$id, 'Laboratorist']);
        redirect('/hms/public/laboratorists/index.php?msg=deleted');
    }
}
...
// Queries - only Laboratorist accounts
...
if ($edit_id) {
    ...
    $st->execute([$edit_id, 'Laboratorist']);
    $edit_row = $st->fetch();
    if (!$edit_row) {
        $edit_row = null;
    }
...
?>
	<h2 class="mb-4">Manage Laboratorist</h2>

	<?php if (get('msg') === 'created') { ?>
		<div class="alert alert-success">Laboratorist created successfully.</div>
	<?php } elseif (get('msg') === 'updated') { ?>
		<div class="alert alert-success">Laboratorist updated successfully.</div>
	<?php } elseif (get('msg') === 'deleted') { ?>
		<div class="alert alert-warning">Laboratorist deleted.</div>
	<?php } ?>
	<?php if ($errors) { ?>
	...
	<ul class="nav nav-tabs mb-3" role="tablist">
		<li class="nav-item" role="presentation">
			<button class="nav-link <?php echo $edit_row ? '' : 'active'; ?>" data-bs-toggle="tab" data-bs-target="#tab-list" type="button" role="tab">Laboratorist List</button>
		</li>
		<li class="nav-item" role="presentation">
			<button class="nav-link <?php echo $edit_row ? '' : ''; ?>" data-bs-toggle="tab" data-bs-target="#tab-add" type="button" role="tab">Add Laboratorist</button>
		</li>
		<?php if ($edit_row) { ?>
		<li class="nav-item" role="presentation">
	...
	<div class="tab-content">
		<div class="tab-pane fade <?php echo $edit_row ? '' : 'show active'; ?>" id="tab-list" role="tabpanel">
			<div class="d-flex justify-content-between align-items-center mb-3">
				...
				<a href="#tab-add" class="btn btn-primary" data-bs-toggle="tab">+ Add Laboratorist</a>
			</div>
			...
			<div class="table-responsive">
				<table class="table table-striped align-middle">
					...
					<tbody>
						<?php $i = $offset + 1; foreach ($laboratorists as $l) { ?>
						<tr>
							<td><?php echo $i++; ?></td>
							<td><?php echo sanitize($l['username']); ?></td>
							<td><?php echo sanitize($l['name']); ?></td>
							<td><?php echo sanitize($l['specialty_area'] ?: 'N/A'); ?></td>
							<td><?php echo sanitize($l['certification'] ?: 'N/A'); ?></td>
							<td>
								<a class="btn btn-sm btn-outline-primary" href="/hms/public/laboratorists/index.php?edit=<?php echo (int)$l['id']; ?>"><i class="bi bi-pencil"></i></a>
								<form action="/hms/public/laboratorists/index.php" method="post" class="d-inline" onsubmit="return confirm('Delete this laboratorist account?');">
						...
						<?php } ?>
						<?php if (!$laboratorists) { ?>
						<tr><td colspan="6" class="text-center text-muted">No laboratorists found</td></tr>
						<?php } ?>
					</tbody>
				</table>
			</div>
		...
		<div class="tab-pane fade" id="tab-add" role="tabpanel">
			<div class="card">
				<div class="card-header">Add Laboratorist</div>
				<div class="card-body">
					<form method="post">
						...
						<div class="mb-3">
							...
							<input type="text" class="form-control" name="certification" placeholder="Laboratory certification">
						</div>
						<div class="mb-3">
						...
						<div class="mb-3">
							...
							<input type="text" class="form-control" name="desk_number" placeholder="Lab station number">
						</div>
		...
		<div class="tab-pane fade show active" id="tab-edit" role="tabpanel">
			<div class="card">
				<div class="card-header">Edit Laboratorist</div>
				<div class="card-body">
					<form method="post">
						...
						<div class="mb-3">
							<label class="form-label">Specialty Area</label>
							<input type="text" class="form-control" name="specialty_area" value="<?php echo sanitize($edit_row['specialty_area']); ?>" placeholder="Hematology, Biochemistry, etc.">
						</div>
						<div class="mb-3">
							<label class="form-label">Certification</label>
							<input type="text" class="form-control" name="certification" value="<?php echo sanitize($edit_row['certification']); ?>" placeholder="Laboratory certification">
						</div>
						<div class="mb-3">
							<label class="form-label">Shift</label>
						...
						<div class="mb-3">
							<label class="form-label">Desk Number</label>
							<input type="text" class="form-control" name="desk_number" value="<?php echo sanitize($edit_row['desk_number']); ?>" placeholder="Lab station number">
						</div>
						<button class="btn btn-primary">Update</button>
						<a href="/hms/public/laboratorists/index.php" class="btn btn-outline-secondary">Cancel</a>
					</form>
				</div>
			</div>
		</div>
