...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        if (!$errors) {
            ...
            $stmt->execute([$username, $name, $hash, 'Nurse', $license_number, $certification, $shift, $desk_number, $specialty_area]);
            redirect('/hms/public/nurses/index.php?msg=created');
        }
        ...
        if (!$errors) {
            ...
            redirect('/hms/public/nurses/index.php?msg=updated');
        }
    } elseif ($action === 'delete') {
        $id = (int)post('id');
        $stmt = $db->prepare('DELETE FROM users WHERE id = ? AND account_type = ?');
        $stmt->execute([$id, 'Nurse']);
        redirect('/hms/public/nurses/index.php?msg=deleted');
    }
}
...
?>
	<h2 class="mb-4">Manage Nurse</h2>

	<?php if (get('msg') === 'created') { ?>
		<div class="alert alert-success">Nurse created successfully.</div>
	<?php } elseif (get('msg') === 'updated') { ?>
		<div class="alert alert-success">Nurse updated successfully.</div>
	<?php } elseif (get('msg') === 'deleted') { ?>
		<div class="alert alert-warning">Nurse deleted.</div>
	<?php } ?>
	<?php if ($errors) { ?>
	...
	<ul class="nav nav-tabs mb-3" role="tablist">
		<li class="nav-item" role="presentation">
			<button class="nav-link <?php echo $edit_row ? '' : 'active'; ?>" data-bs-toggle="tab" data-bs-target="#tab-list" type="button" role="tab">Nurse List</button>
		</li>
		<li class="nav-item" role="presentation">
			<button class="nav-link <?php echo $edit_row ? '' : ''; ?>" data-bs-toggle="tab" data-bs-target="#tab-add" type="button" role="tab">Add Nurse</button>
		</li>
		<?php if ($edit_row) { ?>
		<li class="nav-item" role="presentation">
	...
	<div class="tab-content">
		<div class="tab-pane fade <?php echo $edit_row ? '' : 'show active'; ?>" id="tab-list" role="tabpanel">
			<div class="d-flex justify-content-between align-items-center mb-3">
				...
				<a href="#tab-add" class="btn btn-primary" data-bs-toggle="tab">+ Add Nurse</a>
			</div>
			...
			<div class="table-responsive">
				<table class="table table-striped align-middle">
					...
					<tbody>
						<?php $i = $offset + 1; foreach ($nurses as $n) { ?>
						<tr>
							<td><?php echo $i++; ?></td>
							<td><?php echo sanitize($n['username']); ?></td>
							<td><?php echo sanitize($n['name']); ?></td>
							<td><?php echo sanitize($n['certification'] ?: 'N/A'); ?></td>
							<td><?php echo sanitize($n['shift'] ?: 'N/A'); ?></td>
							<td>
								<a class="btn btn-sm btn-outline-primary" href="/hms/public/nurses/index.php?edit=<?php echo (int)$n['id']; ?>"><i class="bi bi-pencil"></i></a>
								<form action="/hms/public/nurses/index.php" method="post" class="d-inline" onsubmit="return confirm('Delete this nurse account?');">
						...
						<?php } ?>
						<?php if (!$nurses) { ?>
						<tr><td colspan="6" class="text-center text-muted">No nurses found</td></tr>
						<?php } ?>
					</tbody>
				</table>
			</div>
		...
		<div class="tab-pane fade" id="tab-add" role="tabpanel">
			<div class="card">
				<div class="card-header">Add Nurse</div>
				<div class="card-body">
					<form method="post">
						...
						<div class="mb-3">
							...
							<input type="text" class="form-control" name="certification" placeholder="Nursing certification">
						</div>
						<div class="mb-3">
						...
						<div class="mb-3">
							...
							<input type="text" class="form-control" name="specialty_area" placeholder="e.g., Emergency, ICU, Pediatrics">
						</div>
			...
			<div class="card">
				...
				<div class="card-body">
					<form method="post">
						...
						<div class="mb-3">
							<label class="form-label">Certification</label>
							<input type="text" class="form-control" name="certification" value="<?php echo sanitize($edit_row['certification']); ?>" placeholder="Nursing certification">
						</div>
						<div class="mb-3">
							<label class="form-label">Shift</label>
						...
						<div class="mb-3">
							<label class="form-label">Specialty Area</label>
							<input type="text" class="form-control" name="specialty_area" value="<?php echo sanitize($edit_row['specialty_area']); ?>" placeholder="e.g., Emergency, ICU, Pediatrics">
						</div>
						<button class="btn btn-primary">Update</button>
						<a href="/hms/public/nurses/index.php" class="btn btn-outline-secondary">Cancel</a>
					</form>
				</div>
			</div>
		</div>
